...
def create_app():
    ...
    trello_api2 = TrelloAPI2()

    @app.route("/", methods=['GET', 'POST'])
    def index():
        item_view_model = TrelloViewModel(trello_api2.get_all_trello_items())  
        return render_template('index.html', view_model=item_view_model)

    @app.route("/complete_item/<string:id>")
    def complete_item(id):
        trello_api2.markid_item_done(id)
        return redirect('/')

    @app.route("/doing_item/<string:id>")
    def doing_item(id):
        trello_api2.markid_item_doing(id)
        return redirect('/')

    @app.route("/uncomplete_item/<string:id>")
    def uncomplete_item(id):
        trello_api2.markid_item_undone(id)
        return redirect('/')
    ...
    def add_todo_item():
        ...
        if isvalid(new_todo_item):
            trello_api2.add_item(new_todo_item)
        return redirect('/')
